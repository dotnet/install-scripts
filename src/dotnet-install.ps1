...
#>
[cmdletbinding()]
[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSReviewUnusedParameter', 'Architecture', Justification = 'false positive due to https://github.com/PowerShell/PSScriptAnalyzer/issues/1472')]
[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSReviewUnusedParameter', 'NoPath', Justification ='false positive due to https://github.com/PowerShell/PSScriptAnalyzer/issues/1472')]
[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSReviewUnusedParameter', 'AzureFeed', Justification = 'false positive due to https://github.com/PowerShell/PSScriptAnalyzer/issues/1472')]
[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSReviewUnusedParameter', 'UncachedFeed', Justification = 'false positive due to https://github.com/PowerShell/PSScriptAnalyzer/issues/1472')]
[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSReviewUnusedParameter', 'FeedCredential', Justification = 'false positive due to https://github.com/PowerShell/PSScriptAnalyzer/issues/1472')]
[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSReviewUnusedParameter', 'ProxyAddress', Justification = 'false positive due to https://github.com/PowerShell/PSScriptAnalyzer/issues/1472')]
[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSReviewUnusedParameter', 'ProxyUseDefaultCredentials', Justification = 'false positive due to https://github.com/PowerShell/PSScriptAnalyzer/issues/1472')]
...
function Measure-Action($name, $block) {
    ...
    Show-Verbose "⏱ Action '$name' took $totalSeconds seconds"
}
...
function Get-Remote-File-Size($zipUri) {
    try {
        ...
        Show-Verbose "Content-Length header was not extracted for $zipUri."
    }

    return $null
}

function Show-Invocation($Invocation) {
    ...
    Show-Verbose "$command $argsString"
}
...
function Get-NormalizedChannel([string]$Channel) {
    ...
    if ($Channel.Contains("Current")) {
        Show-Warning 'Value "Current" is deprecated for -Channel option. Use "STS" instead.'
    }

    if ($Channel.StartsWith('release/')) {
        Show-Warning 'Using branch name with -Channel option is no longer supported with newer releases. Use -Quality option with a channel in X.Y format instead, such as "-Channel 5.0 -Quality Daily."'
    }
...
function Add-Assembly([string] $Assembly) {
    try {
...
function GetHTTPResponse([Uri] $Uri, [bool]$HeaderOnly, [bool]$DisableRedirect, [bool]$DisableFeedCredential)
{
    ...
    $downloadScript = {
        ...
        try {
            ...
            Add-Assembly -Assembly System.Net.Http
            ...
            if(-not $ProxyAddress) {
                try {
                    ...
                    $ProxyAddress = $null
                    Show-Verbose("Exception ignored: $_.Exception.Message - moving forward...")
                }
            }
...
function Get-Version-From-LatestVersion-File([string]$AzureFeed, [string]$Channel) {
    ...
    Show-Verbose "Constructed latest.version URL: $VersionFileUrl"

    try {
        $Response = GetHTTPResponse -Uri $VersionFileUrl
    }
    catch {
        Show-Verbose "Failed to download latest.version file."
        throw
    }
...
function Read-Jsonfile-For-Version([string]$JSonFile) {
    Show-Invocation $MyInvocation
    ...
    try {
        ...
        Show-Error "Json file unreadable: '$JSonFile'"
        throw
    }
    if ($JSonContent) {
        try {
            $JSonContent.PSObject.Properties | ForEach-Object {
                ...
                if ($PropertyName -eq "version") {
                    $Version = $_.Value
                    Show-Verbose "Version = $Version"
                }
            }
        }
        catch {
            Show-Error "Unable to parse the SDK node in '$JSonFile'"
            throw
        }
    }
    else {
...
function Get-Specific-Version-From-Version([string]$AzureFeed, [string]$Channel, [string]$Version, [string]$JSonFile) {
    ...
    if (-not $JSonFile) {
        ...
        return Read-Jsonfile-For-Version $JSonFile
    }
}
...
function Get-Download-Link([string]$AzureFeed, [string]$SpecificVersion, [string]$CLIArchitecture) {
    ...
    Show-Verbose "Constructed primary named payload URL: $PayloadURL"
...
function Get-LegacyDownload-Link([string]$AzureFeed, [string]$SpecificVersion, [string]$CLIArchitecture) {
    ...
    Show-Verbose "Constructed legacy named payload URL: $PayloadURL"
...
function Get-Product-Version-Url([string]$AzureFeed, [string]$SpecificVersion, [string]$PackageDownloadLink, [bool]$Flattened) {
    ...
    Show-Verbose "Constructed productVersion link: $ProductVersionTxtURL"
...
function Get-ProductVersionFromDownloadLink([string]$PackageDownloadLink, [string]$SpecificVersion)
{
    ...
    if ($filenameParts.Length -gt 2)
    {
        ...
        Show-Verbose "Extracted product version '$productVersion' from download link '$PackageDownloadLink'."
    }
    else {
        Show-Verbose "Using the default value '$SpecificVersion' as the product version."
...
function Find-Installed-Dotnet-Package([string]$InstallRoot, [string]$RelativePathToPackage, [string]$SpecificVersion) {
    Show-Invocation $MyInvocation

    $DotnetPackagePath = Join-Path -Path $InstallRoot -ChildPath $RelativePathToPackage | Join-Path -ChildPath $SpecificVersion
    Show-Verbose "Find-Installed-Dotnet-Package: DotnetPackagePath=$DotnetPackagePath"
...
function Get-List-Of-Directories-And-Versions-To-Unpack-From-Dotnet-Package([System.IO.Compression.ZipArchive]$Zip, [string]$OutPath) {
    ...
    Show-Verbose "Directories to unpack: $values"
...
#           ...         # extract content
function Expand-Dotnet-Package([string]$ZipPath, [string]$OutPath) {
    Show-Invocation $MyInvocation

    Add-Assembly -Assembly System.IO.Compression.FileSystem
    ...
    {
        Show-Error "Failed to extract package. Exception: $_"
        throw;
    }
    finally {
        if ($null -ne $Zip) {
...
function SafeRemoveFile($Path) {
    try {
        if (Test-Path $Path) {
            Remove-Item $Path
            Show-Verbose "The temporary file `"$Path`" was removed."
        }
        else {
            Show-Verbose "The temporary file `"$Path`" does not exist, therefore is not removed."
        }
    }
    catch {
        Show-Warning "Failed to remove the temporary file: `"$Path`", remove it manually."
    }
}

function Add-Sdk-InstallRoot-To-Path([string]$InstallRoot) {
    ...
    if (-Not $NoPath) {
        ...
        if (-Not $env:path.Contains($SuffixedBinPath)) {
            ...
            Show-Verbose "Current process PATH already contains `"$BinPath`""
        }
    }
    else {
...
function Get-AkaMSDownloadLink([string]$Channel, [string]$Quality, [bool]$Internal, [string]$Product, [string]$Architecture) {
    ...
    Show-Verbose  "Constructed aka.ms link: '$akaMsLink'."
    ...
    for ($maxRedirections = 9; $maxRedirections -ge 0; $maxRedirections--)
    {
        ...
        Show-Verbose "Received response:`n$Response"

        if ([string]::IsNullOrEmpty($Response)) {
            Show-Verbose "The link '$akaMsLink' is not valid: failed to get redirect location. The resource is not available."
            return $null
        }
        ...
        if  ($Response.StatusCode -eq 301)
        {
            try {
                ...
                if ([string]::IsNullOrEmpty($akaMsDownloadLink)) {
                    Show-Verbose "The link '$akaMsLink' is not valid: server returned 301 (Moved Permanently), but the headers do not contain the redirect location."
                    return $null
                }

                Show-Verbose "The redirect location retrieved: '$akaMsDownloadLink'."
                # This may yet be a link to another redirection. Attempt to retrieve the page again.
                $akaMsLink = $akaMsDownloadLink
                continue
            }
            catch {
                Show-Verbose "The link '$akaMsLink' is not valid: failed to get redirect location."
                return $null
            }
        }
        ...
        Show-Verbose "The link '$akaMsLink' is not valid: failed to retrieve the redirection location."
        return $null
    }

    Show-Verbose "Aka.ms links have redirected more than the maximum allowed redirections. This may be caused by a cyclic redirection of aka.ms links."
    return $null
...
function Get-AkaMsLink-And-Version([string] $NormalizedChannel, [string] $NormalizedQuality, [bool] $Internal, [string] $ProductName, [string] $Architecture) {
    ...
    if ([string]::IsNullOrEmpty($AkaMsDownloadLink)){
        if (-not [string]::IsNullOrEmpty($NormalizedQuality)) {
            ...
            Show-Error "Failed to locate the latest version in the channel '$NormalizedChannel' with '$NormalizedQuality' quality for '$ProductName', os: 'win', architecture: '$Architecture'."
            Show-Error "Refer to: https://aka.ms/dotnet-os-lifecycle for information on .NET Core support."
            throw "aka.ms link resolution failure"
        }
        Show-Verbose "Falling back to latest.version file approach."
        return ($null, $null, $null)
    }
    else {
        Show-Verbose "Retrieved primary named payload URL from aka.ms link: '$AkaMsDownloadLink'."
        Show-Verbose  "Downloading using legacy url will not be attempted."
        ...
        if ($pathParts.Length -ge 2) {
            ...
            Show-Error "Failed to extract the version from download link '$AkaMsDownloadLink'."
            return ($null, $null, $null)
        }
        ...
        Show-Verbose "Product version: '$EffectiveVersion'."
...
function Assert-Install-Directory {
...
Show-Verbose "Note that the intended use of this script is for Continuous Integration (CI) scenarios, where:"
Show-Verbose "- The SDK needs to be installed without user interaction and without admin rights."
Show-Verbose "- The SDK installation doesn't need to persist across multiple CI runs."
Show-Verbose "To set up a development environment or to run apps, use installers rather than this script. Visit https://dotnet.microsoft.com/download to get the installer.`r`n"
...
Measure-Action "Product discovery" {
    ...
    Show-Verbose "Normalized quality: '$NormalizedQuality'"
    $script:NormalizedChannel = Get-NormalizedChannel $Channel
    Show-Verbose "Normalized channel: '$NormalizedChannel'"
    $script:NormalizedProduct = Get-NormalizedProduct $Runtime
    Show-Verbose "Normalized product: '$NormalizedProduct'"
    $script:FeedCredential = ValidateFeedCredential $FeedCredential
}

$InstallRoot = Resolve-Installation-Path $InstallDir
Show-Verbose "InstallRoot: $InstallRoot"
    ...
    if ($null -ne $DownloadLink) {
        ...
        if (-Not $DryRun) {
            ...
            if (Find-Installed-Dotnet-Package -InstallRoot $InstallRoot -RelativePathToPackage $dotnetPackageRelativePath -SpecificVersion $EffectiveVersion)
            {
                ...
                Add-Sdk-InstallRoot-To-Path -InstallRoot $InstallRoot
                return
            }
        }
    }
...
if ([string]::IsNullOrEmpty($NormalizedQuality) -and 0 -eq $DownloadLinks.count)
{
    foreach ($feed in $feeds) {
        ...
        {
            Show-Verbose "Failed to acquire download links from feed $feed. Exception: $_"
        }
    }
}
...
Measure-Action "Installation directory preparation" { Assert-Install-Directory }

Show-Verbose "Zip path: $ZipPath"
...
{
    Show-Verbose "Downloading `"$($link.type)`" link $($link.downloadLink)"

    try {
        Measure-Action "Package download" { DownloadFile -Source $link.downloadLink -OutPath $ZipPath }
        Show-Verbose "Download succeeded."
        ...
        Show-Verbose "Download failed with status code $StatusCode. Error message: $ErrorMessage"
...
Measure-Action "Package extraction" { Expand-Dotnet-Package -ZipPath $ZipPath -OutPath $InstallRoot }
...
if ($DownloadedLink.effectiveVersion -Match "rtm" -or $DownloadedLink.effectiveVersion -Match "servicing") {
    ...
    Show-Verbose "Checking installation: version = $ReleaseVersion"
    $isAssetInstalled = Find-Installed-Dotnet-Package -InstallRoot $InstallRoot -RelativePathToPackage $dotnetPackageRelativePath -SpecificVersion $ReleaseVersion
}

#  Check if the SDK version is installed.
if (!$isAssetInstalled) {
    Show-Verbose "Checking installation: version = $($DownloadedLink.effectiveVersion)"
    $isAssetInstalled = Find-Installed-Dotnet-Package -InstallRoot $InstallRoot -RelativePathToPackage $dotnetPackageRelativePath -SpecificVersion $DownloadedLink.effectiveVersion
}

# Version verification failed. More likely something is wrong either with the downloaded content or with the verification algorithm.
if (!$isAssetInstalled) {
    Show-Error "Failed to verify the version of installed `"$assetName`".`nInstallation source: $($DownloadedLink.downloadLink).`nInstallation location: $InstallRoot.`nReport the bug at https://github.com/dotnet/install-scripts/issues."
...
Measure-Action "Setting up shell environment" { Add-Sdk-InstallRoot-To-Path -InstallRoot $InstallRoot }
